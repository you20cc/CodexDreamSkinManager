...
((cssText, artDataUrl, themeConfig) => {
  ...
  const applyArtMetadata = (root) => {
    ...
    const shell = root.getAttribute(SHELL_ATTR) || detectShellAppearance();
    const homeDefault = shell === "light" ? 0.93 : 0.91;
    const taskDefault = shell === "light" ? 0.18 : 0.22;
    const homeOpacity = typeof ART.homeOpacity === "number" ? clamp(ART.homeOpacity, 0, 1) : homeDefault;
    const taskBaseOpacity = typeof ART.taskOpacity === "number" ? clamp(ART.taskOpacity, 0, 1) : taskDefault;
    const taskOpacity = taskMode === "off"
      ? 0
      : clamp(taskBaseOpacity * (taskMode === "banner" ? 1.8 : 1), 0, 0.95);
    const homeScale = homeDefault > 0 ? homeOpacity / homeDefault : 0;
    const taskScale = (taskMode === "banner" ? Math.min(0.95, taskDefault * 1.8) : taskDefault) > 0
      ? taskOpacity / (taskMode === "banner" ? Math.min(0.95, taskDefault * 1.8) : taskDefault)
      : 0;
    const alpha = (value) => String(Number(clamp(value, 0, 1).toFixed(4)));
    const homeRgb = shell === "light" ? "--ds-panel-rgb" : "--ds-bg-rgb";
    const homeSidebarRgb = "--ds-panel-rgb";
    const homeBase = shell === "light"
      ? { edge: .44, mid: .28, far: .14, sidebar: .48, chrome: .34, composer: .46 }
      : { edge: .40, mid: .26, far: .16, sidebar: .46, chrome: .28, composer: .44 };
    const taskBase = shell === "light"
      ? { edge: .86, mid: .78, far: .66, sidebar: .72 }
      : { edge: .82, mid: .74, far: .60, sidebar: .70 };
    ...
    setStyleProperty(root, "--dream-home-opacity", alpha(homeOpacity));
    setStyleProperty(root, "--dream-task-opacity", alpha(taskOpacity));
    setStyleProperty(root, "--ds-immersive-edge", `rgb(var(${homeRgb}) / ${alpha(homeBase.edge * homeScale)})`);
    setStyleProperty(root, "--ds-immersive-mid", `rgb(var(${homeRgb}) / ${alpha(homeBase.mid * homeScale)})`);
    setStyleProperty(root, "--ds-immersive-far", `rgb(var(${homeRgb}) / ${alpha(homeBase.far * homeScale)})`);
    setStyleProperty(root, "--ds-immersive-sidebar", `rgb(var(${homeSidebarRgb}) / ${alpha(homeBase.sidebar * homeScale)})`);
    setStyleProperty(root, "--ds-immersive-chrome", `rgb(var(${homeSidebarRgb}) / ${alpha(homeBase.chrome * homeScale)})`);
    setStyleProperty(root, "--ds-immersive-composer", `rgb(var(${homeSidebarRgb}) / ${alpha(homeBase.composer * homeScale)})`);
    setStyleProperty(root, "--ds-task-immersive-sidebar", `rgb(var(${homeSidebarRgb}) / ${alpha(taskBase.sidebar * taskScale)})`);
    setStyleProperty(root, "--ds-task-immersive-edge", `rgb(var(${homeRgb}) / ${alpha(taskBase.edge * taskScale)})`);
    setStyleProperty(root, "--ds-task-immersive-mid", `rgb(var(${homeRgb}) / ${alpha(taskBase.mid * taskScale)})`);
    setStyleProperty(root, "--ds-task-immersive-far", `rgb(var(${homeRgb}) / ${alpha(taskBase.far * taskScale)})`);
  };
